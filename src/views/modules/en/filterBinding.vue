  ...
  <div class="mod-config">
      <el-card  v-for="(item, index) in filterBindingData" :key="index" class="box-card"  style="margin-top: 10px">
        <div slot="header" class="clearfix">
          ...
          <el-button style="float: right; padding: 3px 0" type="text" @click="saveFilterBindings(item.goodsCategoryEntity, index)">保存</el-button>
        </div>
        <!-- <div v-for="o in 4" :key="o" class="text item">
          {{'列表内容 ' + o }}
        </div> -->
        <el-checkbox-group v-model="checkList[index]"  @change="handleChange">
          <el-checkbox v-for="(item1, index1) in item.filterEntityList" :key="index1" :label="item1.id">{{item1.label}}</el-checkbox>
        </el-checkbox-group>
      </el-card>
  </div>
</template>
...
<script>
  ...
  export default {
    created () {
      ...
      this.$http({
        url: this.$http.adornUrl('/generator/filter/en/filterBinding'),
        method: 'get'
      }).then(({data}) => {
        console.log(data)
        ...
        this.filterBindingData.map((item, index) => {
          if (item.ids !== null) {
            this.$set(this.checkList, index, item.ids)
          } else {
    ...
    methods: {
      saveFilterBindings (goodsCategoryEntity, index) {
        // console.log('123456789')
        // console.log(goodsCategoryEntity)
        // console.log(this.checkList[index])
        this.$http({
          url: this.$http.adornUrl(`/generator/filter/en/saveFilterBindings`),
          method: 'post',
          data: this.$http.adornData({
            'categoryid': goodsCategoryEntity.id,
            'filterids': this.checkList[index].toString()
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
            this.$message({
