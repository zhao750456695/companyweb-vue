...
<div>
  <el-dialog
    ...
    <el-form-item label="简介" prop="desc">
      <el-input type="textarea" :rows="2" :autosize="{ minRows: 6}" v-model="dataForm.desc" placeholder=""></el-input>
    </el-form-item>
    ...
    methods: {
      ...
      uploadListPicture (item) {
        ...
        this.$http({
          ...
          item.onSuccess()
          // console.log(data)
          this.listPicture.push({url: data})
          ...
          if (this.dataForm.id) {
            this.$http({
              ...
              if (data && data.code === 0) {
                ...
                let d = data.news.updated
                let date = new Date(d)
                this.value = date
                this.editorData = this.dataForm.content
                this.listPicture = []
                this.listPicture.push({url: data.news.cover})
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            console.log(this.listPicture[0].url, 99999999999)
            // console.log(CKEditor.component.instance.getData())
            if (!this.dataForm.id) {
              this.$http({
                url: this.$http.adornUrl(`/generator/news/save`),
                method: 'post',
              ...
              this.$http({
                url: this.$http.adornUrl(`/generator/news/update`),
                method: 'post',
                data: this.$http.adornData({
                  'id': this.dataForm.id,
                  'title': this.dataForm.title || undefined,
